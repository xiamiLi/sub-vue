<template>
  <div id="app">
    ...
    <div>
      ...
      <p v-if="isInQiankun">I am from qiankun.Its userDetail：<code> {{ JSON.stringify(userDetail.uid) }}</code></p>
      <p v-else>vuex的`global module`的user state：<code> {{ JSON.stringify(user) }}</code></p>
    </div>
    <div class="btns">
      <template v-if="isInQiankun">
        ...
        <button @click="handleTestApi">test api</button>
      </template>
...
import { queryUserDetail } from './api/index'
export default {
  computed: {
    // 通过global获取user的信息
    ...mapState('global', {
